...
exports.getUserNotifications = onRequest(async (request, response) => {
  ...
  try {
    ...
    response.status(500).json({ error: `Failed to get user notifications: ${err}` });
  }
});

exports.fetchWordPressData = onRequest(async (request, response) => {
  cors(request, response, async () => {
    const { userToken, siteUrl } = request.query;

    try {
      // Fetch categories, posts, and pages from the WordPress API
      const [categories, posts, pages, products] = await Promise.all([
        axios.get(`${siteUrl}/wp-json/wp/v2/categories`),
        axios.get(`${siteUrl}/wp-json/wp/v2/posts`),
        axios.get(`${siteUrl}/wp-json/wp/v2/pages`),
        axios.get(`${siteUrl}/wp-json/wc/v3/products`), // Fetch WooCommerce products
      ...
      categories.data.forEach((category) => {
        ...
        batch.set(docRef, { ...category, userToken, siteUrl });
      });

      posts.data.forEach((post) => {
        const docRef = db.collection('userWpSiteSitePosts').doc();
        batch.set(docRef, { ...post, userToken, siteUrl});
      });

      pages.data.forEach((page) => {
        const docRef = db.collection('userWpSitePages').doc();
        batch.set(docRef, { ...page, userToken  , siteUrl});
      });

      products.data.forEach((product) => {
        const docRef = db.collection('userWpSiteProducts').doc();
        batch.set(docRef, { ...product, userToken , siteUrl});
      });

      await batch.commit();

      response.status(200).json({ message: 'WordPress data fetched and stored successfully.' });
    } catch (err) {
      response.status(500).json({ error: `Failed to fetch WordPress data: ${err}` });
    }
  });
});
...
async function seedCollections() {
  ...
  const goalsListData = [
    ...
    },
    {
      ...
      description: "Create content that improves customer service experiences by providing helpful and detailed support information.",
      icon: ["fas", "headset"],
    },
  ];
  ...
  const tonesListData = [
    ...
    },
    {
      ...
      description: "Warm and inviting, suitable for casual or personal communications, mimics natural speech patterns to feel more personal and relatable.",
      icon: ["fas", "face-smile"],
    },
    {
      id: 4,
  ...
  const platformsListData = [
    {
      ...
      description: "Content tailored for social media platforms like Twitter, Facebook, Instagram, Linkedin etc.",
    ...
    },
    {
      ...
      description: "Detailed blog articles and persuasive product descriptions, suitable for websites or e-commerce platforms, provided in HTML format.",
      icon: ["fas", "globe"],
    },
    {
      id: 5,
    ...
    },
    {
      ...
      description: "Well-researched and structured content for academic papers or assignments.",
      icon: ["fas", "user-graduate"],
    },
  ];
