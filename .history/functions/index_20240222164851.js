...
exports.generateContent = onRequest(async (request, response) => {
  cors(request, response, async () => {
    try {
      ...
      await insertToNotificationsList(userToken, "New content generation request - " + promptParams.title);
      ...
      process.nextTick(async () => {
        try {
          ...
          await insertToNotificationsList(userToken, "Content generated successfully - " + title);
        } catch (error) {
          await docRef.update({
            status: "failed",
          });
          await insertToNotificationsList(userToken, "Failed to generate content", error);
...
exports.fetchWordPressData = onRequest(async (request, response) => {
  cors(request, response, async () => {
    const { userToken, url } = request.query;
    ...
    process.nextTick(async () => {
      try {
        ...
        const [categories, posts, pages, products] = await Promise.all([
        ...
        await insertToNotificationsList(userToken, "WordPress category data fetched successfully");
        ...
        await insertToNotificationsList(userToken, "WordPress posts data fetched successfully");
        ...
        await insertToNotificationsList(userToken, "WordPress pages data fetched successfully");

        products.data.forEach((product) => {
          const docRef = db.collection("userWpSiteProducts").doc();
          batch.set(docRef, { ...product, userToken, url });
        });
        await insertToNotificationsList(userToken, "WordPress products data fetched successfully");

        await batch.commit();

        response
          .status(200)
          .json({ message: "WordPress data fetched and stored successfully." });
      } catch (err) {
        await insertToNotificationsList(userToken, "Failed to fetch WordPress data", err);
        response
          .status(500)
          .json({ error: `Failed to fetch WordPress data: ${err}` });
      }
    });
  });
});
...
exports.createWpPost = onRequest(async (request, response) => {
  ...
  try {
    await generateWpPost(userToken);
...
async function generateWpPost(userToken) {
  try {
    const userWpSiteRef = db.collection("userWpSite");
    ...
    const userWpSitePostsRef = db.collection("userWpSitePosts");
    ...
    // const randomPage = pageSnapshot.docs[Math.floor(Math.random() * pageSnapshot.size)];

    const { title: postTitle, content: postContent } = randomPost.data();
    // const { title: pageTitle, content: pageContent } = randomPage.data();
    ...
    await insertToNotificationsList(userToken, "New wordpress post created - " + title);
...
async function postDraftToWordPress(
  ...
  try {
    const generatedForWPRef = db.collection("generatedContent").doc();
    await generatedForWPRef.set({
      wordpress:true,
      url,
      adminToken,
      title,
    ...
    if (response.status === 201) {
      ...
      await insertToNotificationsList(userToken, "New wordpress post draft created - " + title);
