...
async function handleWordPressApiRequest(method, endpoint, data, token, siteUrl) {
  try {
    ...
    throw new functions.https.HttpsError('unknown', 'Failed to process WordPress API request', error);
  }
}
...
async function getWpCredentials(token, siteUrl) {
  const db = admin.firestore();
  try {
    const snapshot = await db.collection('WpUserSite')
                              .where('token', '==', token)
                              .where('url', '==', siteUrl)
                              .limit(1)
                              .get();
    ...
    if (snapshot.empty) {
      ...
      throw new Error('No matching credentials found for provided token and site URL.');
    }
...
exports.scheduledFetchPosts = functions.pubsub.schedule('every 5 minutes').onRun(async (context) => {
  const token = 'specific-token'; // Normally, you would fetch or define these dynamically or securely
  const siteUrl = 'https://example.com';
  try {
    const posts = await handleWordPressApiRequest('get', '/posts', { per_page: 10 }, token, siteUrl);
...
exports.wphooks = functions.https.onRequest(async (req, res) => {
  try {
    // Assuming the data is sent as a JSON body
    const {
      event,
      post_id,
      post_title,
    ...
    const token = req.query.token; // Assume token is still passed as a query parameter
    ...
    if (token === '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') {
      ...
      await webhookRef.set({
        ...
        user_ip,
        user_agent,
        received_at: admin.firestore.FieldValue.serverTimestamp() // Use server timestamp for consistency
      });
