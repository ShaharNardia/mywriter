...
exports.scrape = onRequest(async (req, res) => {
  try {
    const { url } = req.body;
    ...
    let prompt = `Please summarize the important details from the following text and return this in hebrew while the text is ready to use for next prompt, remove any tags or images:\n\n${bodyContent}`;
...
exports.generateContent = onRequest(async (request, response) => {
  try {
    ...
    const openAIResponse = await openAI(prompt, 'https://api.openai.com/v1/chat/completions');
