...
exports.getWordpresspostPageCategoryLists = functions.https.onRequest(async (request, response) => {
  try {
    ...
    const wpSiteCategoriesSnapshot = await db
      ...
      .where("url", "==", url)
      .get();
    const wpSiteCategories = wpSiteCategoriesSnapshot.docs.map((doc) => doc.data());

    response.status(200).json({ wpSitePages, wpSitePosts, wpSiteCategories });
  } catch (error) {
    console.error("Error getting lists from collections:", error);
    response.status(500).json({ error: "Failed to get lists from collections" });
  }
});
