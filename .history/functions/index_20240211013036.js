...
if (purpose === 'api') {
  ...
  return 'the content should be in a html format, design it with bold fonts, headers, paragraphs where neeaded';
}}
...
exports.generateContent = onRequest(async (request, response) => {
  try {
    ...
    await sendEmail(generatedContent,"test", "shahar.nardia@retailsoft.io");
    ...
    // });
    response.status(200).send(`${generatedContent}`);
