...
exports.getContentById = onRequest((request, response) => {
    ...
    try {
      ...
      if (documentSnapshot.exists) {
        ...
        if (documentData.userToken === userToken) {
          response.write(`data: ${JSON.stringify(documentData)}\n\n`);
        } else {
          response.write(`event: error\ndata: ${JSON.stringify({ error: "Unauthorized access" })}\n\n`);
        }
      } else {
        response.write(`event: error\ndata: ${JSON.stringify({ error: "Document not found" })}\n\n`);
      }
    } catch (err) {
      response.write(`event: error\ndata: ${JSON.stringify({ error: `Failed to get document ${err}` })}\n\n`);
    }
  };

  sendDocumentData();

  // Close the connection if the client stops responding
  request.on('close', () => {
    response.end();
  });
});
