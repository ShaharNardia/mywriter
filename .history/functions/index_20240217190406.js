...
exports.getFormSettings = onRequest(async (request, response) => {
  try {
    ...
    if (goalsListCollection.empty || tonesListCollection.empty || platformsListCollection.empty) {
...
const seedCollections = async () => {
  try {
    ...
    const jobsListCollection = db.collection("jobsList");


    // Delete existing collections
    await Promise.all([
      deleteCollection(goalsListCollection),
      deleteCollection(tonesListCollection),
      deleteCollection(platformsListCollection),
    ]);
    ...
    const goalsListData = [
      ...
      },
      {
        ...
        goal: "Entertain",
        description:
          "Entertain the audience with creative and engaging content.",
    ...
    await Promise.all(
      platformsListData.map((data) => platformsListCollection.add(data))
    );
    ...
    const jobTypesListData = [
      {
        ...
        description: "Generate a list of blog title ideas based on trending topics or specified niches every week to inspire your content creation.",
        icon: ['fas', 'lightbulb']
      },
      {
        id: 2,
        jobType: "Generate Social Media Post/s",
        description: "Create ready-to-publish social media posts tailored to your brand's voice and audience, saving you hours of brainstorming and writing.",
        icon: ['fas', 'pencil-alt']
      },
      {
        id: 3,
        jobType: "Generate Email Marketing Idea/s",
        description: "Produce innovative email marketing concepts, including subject lines and content ideas, to enhance your email campaigns.",
        icon: ['fas', 'envelope']
      },
      {
        id: 4,
        jobType: "Generate Educational Content Pieces",
        description: "Generate educational articles, lesson plans, or course materials tailored to specific subjects or learning outcomes.",
        icon: ['fas', 'chalkboard-teacher']
      },
      {
        id: 5,
        jobType: "Produce Product Descriptions ",
        description: "Craft unique and persuasive product descriptions for new or existing products, boosting your e-commerce efforts.",
        icon: ['fas', 'tags']
      },
      {
        id: 6,
        jobType: "Automate Weekly Competitive Content Analysis",
        description: "Receive weekly insights on competitors' content strategies, helping you stay ahead and fine-tune your content marketing.",
        icon: ['fas', 'search']
      },
      {
        id: 7,
        jobType: "Generate SEO Content Reports",
        description: "Automatically analyze and report on the SEO performance of your content, providing actionable insights for optimization.",
        icon: ['fas', 'chart-line']
      },
      {
        id: 8,
        jobType: "Create X Press Releases",
        description: "Automatically generate press releases for your company's milestones, product launches, or events, ready for distribution.",
        icon: ['fas', 'bullhorn']
      },
    ];
    
    await Promise.all(
      jobTypesListData.map((data) => jobsListCollection.add(data))
    );
