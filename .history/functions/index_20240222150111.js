...
exports.registerUserSite = onRequest(async (request, response) => {
  try {
    const { userToken, wordpressUrl, token } = request.body;

    // Check if userToken exists in users collection
    const userSnapshot = await db.collection("users").where("userToken", "==", userToken).get();
...
async function generatePostFromPost(userToken) {
  try {
    //const userWpSitePostsRef = db.collection("userWpSitePosts");
    const userWpSite = db.collection("userWpSite");
    const userPostSnapshot = await userWpSitePostsRef.where("userToken", "==", userToken).get();

    if (userPostSnapshot.empty) {
      throw new Error("User token not found in userWpSitePosts collection");
    }

    const randomPost = userPostSnapshot.docs[Math.floor(Math.random() * userPostSnapshot.size)];
    const { url, content } = randomPost.data();

    const openAIResponse = await openAI(`generate me a post from the category ${category} with the following content: ${content}`);
    const { title, generatedContent, keywords } = openAIResponse.data;
    ...
    postDraftToWordPress(url, adminToken, title, generatedContent, keywords)
...
async function postDraftToWordPress(url, adminToken, title, content, keywords) {
  try {
