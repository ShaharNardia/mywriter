...
export default {
  ...
  setup(_, { emit }) {
    ...
    const companyName = ref(user ? user.companyName : "");
    const sendUpdates = ref(user ? user.sendUpdates : false);
    const email = ref(user ? user.email : "");
    const dialCode = ref(user ? user.dialCode : "");
    const phoneNumber = ref(user ? user.phoneNumber : "");
    const defaultCountry = ref("IL");
    const language = ref(user ? user.language : "hebrew");
    const useEmojis = ref(user ? user.useEmojis : false);
