      ...
      <div class="button-container" v-if="isLoggedIn">
        ...
        <div class="button-pop">
          ...
          <div class="settings-popup" v-if="showSettingsPopupVisible">
            <ul class="menu">
              <li>
                <font-awesome-icon :icon="['fas', 'sync']" /><router-link to="/"
                  >Set auto generate content job</router-link
                >
              </li>
              <li>
                <font-awesome-icon :icon="['fas', 'edit']" /><router-link to="/"
...
export default {
  ...
  watch: {
    isLoggedIn(newVal, oldVal) {
      // Perform actions on login status change
      console.log(`Login status changed from ${oldVal} to ${newVal}`);
      // You might want to call some methods here or update local state
    },
  },
  data() {
    return {
