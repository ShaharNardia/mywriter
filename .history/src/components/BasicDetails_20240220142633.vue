<template>
  <div class="flexed">{{ user.value }}werwer
    <div>
...
export default {
  ...
  setup(_, { emit }) {
    ...
    const firstName = ref(user.value ? user.value.firstName : "");
    const lastName = ref(user.value ? user.value.lastName : "");
    const companyName = ref(user.value ? user.value.companyName : "");
    const sendUpdates = ref(user.value ? user.value.sendUpdates : false);
    const email = ref(user.value ? user.value.email : "");
    const dialCode = ref(user.value ? user.value.dialCode : "");
    const phoneNumber = ref(user.value ? user.value.phoneNumber : "");
    const defaultCountry = ref("IL");
    const language = ref(user.value ? user.value.language : "hebrew");
    const useEmojis = ref(user.value ? user.value.useEmojis : false);
