<template>
  <div>
      ...
      <div class="form-group">
        <button @click="generateContent" class="form-button">Generate Content</button>
      </div>
...
export default {
  ...
  methods: {
   async generateContent() {
      let self = this;
      console.log("Generating content with the following parameters:", self.promptParams);
      let data = {
        promptParams: {
      ...
      try {
      const response = await axios.post("https://generatecontent-ibnrirhwvq-uc.a.run.app", data, {
        headers,
      });
