    ...
    <div v-for="generatedContent in contents" :key="generatedContent.id">
      <router-link :to="`/content-page/${generatedContent.id}`" style="text-decoration: none; color:black">
        <div style="margin:20px 0">
          <div style="margin-bottom: 10px;">{{ generatedContent.title.slice(0,100) }}...</div> 
          <div style="margin-bottom: 10px;">{{ new Date(generatedContent.created.seconds * 1000).toLocaleString() }}</div>
