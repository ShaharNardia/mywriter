  ...
  <div>
    <div v-if="step === 1">
      ...
      <select id="phonePrefix" v-model="phonePrefix">
        <option v-for="country in countries" :key="country.code" :value="country.prefix">
    ...
    <div v-else-if="step === 2">
      ...
      <div v-for="goal in goals" :key="goal.id" @click="selectGoal(goal.id)" :class="{ selected: selectedGoal === goal.id }">
        {{ goal.name }}
      </div>
      <button @click="nextStep">Next</button>
    </div>
    <div v-else-if="step === 3">
      ...
      <p>Disclaimer: This option is only for the format and will not publish or send the content anywhere.</p>
...
export default {
  ...
  methods: {
    ...
    finish() {
      ...
      // Example: this.$axios.put('/api/users', user);
    },
  },
};
</script>
