    ...
    async registerSite() {
      try {
        ...
        const fetchAllItems = async (endpoint, fields) => {
          ...
          while (run) {
            const res = await axios.get(
              `${this.wordpressUrl}${endpoint}?per_page=100&page=${page}`
        ...
        const [categories, posts, pages] = await Promise.all([
          fetchAllItems("/wp-json/wp/v2/categories", "*"),
          fetchAllItems("/wp-json/wp/v2/posts", "*"),
          fetchAllItems("/wp-json/wp/v2/pages", "*"),
        ]);
