...
export default {
  ...
  methods: {
      ...
      onSnapshot(q, (snapshot) => {
        snapshot.docChanges().forEach((change) => {
          if (change.type === "added" || change.type === "modified") {
            ...
            const docRef = doc(db, "users", change.docs[0].id);
            updateDoc(docRef, this.createUserObject())
              .then(() => {
                localStorage.setItem("user", JSON.stringify(this.createUserObject()));
    ...
    async updateUser() {
      ...
      const docRef = doc(db, "users", q.docs[0].id);
    ...
    handleSetBasicDetails(basicDetails) {
      ...
      this.sendUpdates = basicDetails.sendUpdates !== undefined ? basicDetails.sendUpdates : this.sendUpdates;
      this.updateUser();
      this.nextStep();
    },
