<template>
  <div id="app">
    <my-header :is-logged-in="isLoggedIn" :usernameSplit="usernameSplit"></my-header>
...
const usernameSplit = ref(JSON.parse(localStorage.getItem('user'))?.firstName[0] + JSON.parse(localStorage.getItem('user'))?.lastName[0]);
...
function startPollingForLoginStatus() {
  pollingInterval = setInterval(() => {
    ...
    const currentName = JSON.parse(localStorage.getItem('user'))?.firstName[0] + JSON.parse(localStorage.getItem('user'))?.lastName[0];

    if (currentStatus !== isLoggedIn.value) {
      isLoggedIn.value = currentStatus;
    }
    if (currentName !== usernameSplit.value) {
      usernameSplit.value = JSON.parse(currentName)?.firstName[0] + JSON.parse(currentName)?.lastName[0]; 
    }
