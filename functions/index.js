...
const generatePrompt = (promptParams) => {
  ...
  if (generateImage) {
    prompt += `${generateImageConditional}\n\n`;
  }

  if (generateAudio) {
    prompt += `${generateAudioConditional}\n\n`;
  }
...
exports.generateContent = onRequest(async (request, response) => {
  try {
    ...
    const openAIResponse = await openAI(prompt, 'https://api.openai.com/v1/engines/davinci-codex/completions');
